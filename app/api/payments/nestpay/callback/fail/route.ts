...
// Obrađuje neuspešna plaćanja od NestPay
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!hashVerified) {
      return NextResponse.redirect(
        new URL('/payment/failed?error=hash_verification_failed', getBaseUrl(req)),
        303,
      );
    }

    // Providerov potpisani rezultat je autoritativan; naziv endpointa nije.
    // Time validan APPROVED payload poslat na fail URL ne može postati decline.
    ...
    console.error('[NestPay] FAIL callback processing error:', error);

    // Nakon validnog potpisa ne predstavljamo tehnički kvar kao bankarski
    // decline. 503 omogućava provideru/proxy-ju ponovni pokušaj bez promene
    // terminalnog rezultata.
