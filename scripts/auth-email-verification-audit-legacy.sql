-- Aggregate-only audit that is safe on the production baseline before any
-- auth compatibility expands. It intentionally uses only baseline columns.
-- Run with psql; output is strictly: category|count.
...
BEGIN
  IF current_setting('server_encoding') <> 'UTF8' THEN
    RAISE EXCEPTION 'Legacy auth audit requires UTF8 server encoding';
  END IF;
  ...
  FROM (
    VALUES
      ...
      ('EmailVerification', 'token', 'text', 'pg_catalog', 'text', 'NO', NULL),
      ...
      ('PasswordReset', 'token', 'text', 'pg_catalog', 'text', 'NO', NULL),
  ...
  IF invalid_contracts <> 0 THEN
    RAISE EXCEPTION 'Legacy auth audit schema contract is unavailable';
  END IF;

  -- Never let the baseline-only report silently run against an expanded auth
  -- schema where nullable plaintext columns would hide hash-only credentials.
  IF EXISTS (
    SELECT 1
    ...
    RAISE EXCEPTION 'Legacy auth audit refuses an expanded auth schema';
  END IF;
END;
$schema_check$;
...
findings AS (
  ...
  UNION ALL
  SELECT 190, 'tokens.credential.legacy', count(*)::bigint
    FROM public."EmailVerification"
    WHERE token COLLATE "C" ~ '^[0-9a-f]{64}$'
  UNION ALL
  SELECT 200, 'tokens.credential.malformed', count(*)::bigint
    FROM public."EmailVerification"
    WHERE token COLLATE "C" !~ '^[0-9a-f]{64}$'
  UNION ALL
  SELECT 201, 'tokens.created_at.nonfinite', count(*)::bigint
    FROM public."EmailVerification"
    WHERE NOT isfinite("createdAt")
  UNION ALL
  SELECT 202, 'tokens.created_at.in_future', count(*)::bigint
    FROM public."EmailVerification" CROSS JOIN parameters
    WHERE isfinite("createdAt") AND "createdAt" > parameters.audit_at
  UNION ALL
  SELECT 203, 'tokens.expires.nonfinite', count(*)::bigint
    FROM public."EmailVerification"
    WHERE NOT isfinite(expires)
  UNION ALL
  SELECT 204, 'tokens.lifetime.invalid', count(*)::bigint
    FROM public."EmailVerification"
  ...
  UNION ALL
  SELECT 210, 'users.verified.with_leftover_tokens', count(*)::bigint
    FROM user_facts WHERE "emailVerified" IS NOT NULL AND has_token
  UNION ALL
  SELECT 220, 'users.unverified.with_order', count(*)::bigint
    FROM user_facts WHERE "emailVerified" IS NULL AND has_order
  UNION ALL
  SELECT 230, 'users.unverified.with_address', count(*)::bigint
    FROM user_facts WHERE "emailVerified" IS NULL AND has_address
  UNION ALL
  SELECT 240, 'users.unverified.with_wishlist', count(*)::bigint
    FROM user_facts WHERE "emailVerified" IS NULL AND has_wishlist
  UNION ALL
  SELECT 250, 'users.unverified.with_review', count(*)::bigint
    FROM user_facts WHERE "emailVerified" IS NULL AND has_review
  UNION ALL
  SELECT 251, 'users.unverified.with_active_db_session_telemetry_only', count(*)::bigint
    FROM user_facts
    WHERE "emailVerified" IS NULL AND has_active_db_session_telemetry
  UNION ALL
  SELECT 252, 'users.unverified.with_active_password_reset', count(*)::bigint
    FROM user_facts
    WHERE "emailVerified" IS NULL AND has_active_password_reset
  UNION ALL
  SELECT 253, 'users.unverified.with_coupon_usage', count(*)::bigint
    FROM user_facts WHERE "emailVerified" IS NULL AND has_coupon_usage
  UNION ALL
  SELECT 260, 'users.unverified.with_any_activity', count(*)::bigint
    FROM user_facts
    WHERE "emailVerified" IS NULL
      AND (
        has_order
  ...
  UNION ALL
  SELECT 270, 'users.password.invalid_or_unsupported_bcrypt_format', count(*)::bigint
    FROM user_facts WHERE NOT bcrypt_format_valid
  UNION ALL
  SELECT 280, 'users.password.valid_format_but_non_cost_12', count(*)::bigint
